...
from utils.seq_utils   import translate, write_fasta, write_rfasta
from ali_parser.parser import parse_mcoffee_score, parse_mcoffee_aln
...
def main():
    ...
    """
    opts         = get_options()
    sequences    = Sequences (path=opts.fastafile,
                              genetic_code=opts.code, aa=opts.aa)
    ...
    sequences.write(outfile=tmp_dir + '/prot.fasta', item='prot')
    if opts.align == 2:
        sequences.write(outfile=tmp_dir + '/prot.fasta', item='prot',
                        reverse=True)
    # run alignment
...
BINARIES = {'mcoffee' : {'bin': which('t_coffee')},
